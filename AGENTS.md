...
- **Remotely Save（移动端同步）**：配置与操作**仅用户本人**负责；任何 agent 禁止读写 `obsidian/.obsidian/plugins/remotely-save/`（含 `data.json`）、禁止对其执行任何 git 操作；该目录已在根 `.gitignore` 排除。
...
- **每日**：日志写入 `obsidian/每日日志/YYYY-MM-DD.md`，当天结束更新，三栏（今日推进 / 卡点 / 明天要做的），≤10min。
...
## 十二、Agent 交互反馈规范（2026-09-23 起，用户指定）

- **开工先报预期**：多步骤任务先给一句话计划 + 预计耗时（如「先只读排查，约 2-3 分钟」），再开始调工具。
- **阶段必有更新**：每完成一个阶段或静默接近 60-90 秒，必须输出一句状态（在查什么 / 已看到什么 / 下一步）；禁止连续多批工具调用零文字。
- **验证链封顶**：带 `sleep`/`timeout` 的验证链总时长 ≤90 秒；同一验证连续失败 2 次立即停手。
- **卡住即上报**：遇到等待、不确定或需要用户操作，第一句就是「卡在 X，需要你 Y」，附 30 秒可完成的人工检查步骤，不静默重试。
- **验证点优先给人**：优先设计用户 30 秒可完成的检查点（看一眼窗口 / 跑一条命令），agent 侧深探查放其次。
- **收尾必汇报**：阶段/任务结束必须给结果 + 下一步，不允许以工具结果直接结束回合。
