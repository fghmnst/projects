...
> 2026-09-11 起：原 vault 顶层 `TIL/` 目录已并入本库（环境运维 / 工具坑 / 通用知识卡统一在此维护）；库索引见 `somezhishi.md`。
...
- 四个子文件夹：`工具速查/`、`环境排障/`、`编程基础/`、`项目笔记/`
- 子文件夹**不设同名索引**，统一由 `somezhishi.md` 维护分类导航
- 每篇 frontmatter 加 `tags:`，从固定词表选（不新造同义词）：
  - 类型：`速查` / `排障` / `入门` / `指南` / `决策`
  - 领域：`工具` / `obsidian` / `wsl` / `hermes` / `linux` / `git` / `ssh` / `opencode` / `tmux` / `项目`
- 同主题内容可合并为一篇（例：Obsidian 两篇启动排障合并）；单篇超 ~10KB 且含多个独立主题时考虑拆分，拆分后互链
